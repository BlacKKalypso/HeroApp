...
public class HeroService {
    ...
    public Hero getHeroByID(long id) {
        // Fetch API
        // Volley
        String url = Constants.APIConfig.URL_API_DOMAINE +"/"+ id;
        if(Util.isNetworkAvailable()) {
            loadFromAPI(url);
        } else{
            loadFromDBById(id);
    ...
    private void loadFromDBById(long id){
        ArrayList<Hero> heros = (ArrayList<Hero>) DBManager.getInstance().getAppDataBase().heroDao().loadAllByIds(id);
        if (heros.size() > 0) {
            HeroService.this.listener.responseWithSuccess(heros);
